...
*@param col cols
*/
void MazeGenerator::setSize(int row, int col) {
	rows = row;
	cols = col;
}
/*initialize Maze
...
*/
void MazeGenerator::print_console() {
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			if (maze[i][j])
				cout << '#' << ' ';
...
*/
void MazeGenerator::print_file(const char* fileName) {
	ofstream fout;
	fout.open(fileName);
	if (!fout.is_open())
		return;
...
/*punch a maze
*/
void MazeGenerator::punch_a_maze(int num) {
	if(num<0) num = (int)(2.0*sqrt((double)rows)*sqrt((double)cols));	// punch times
	for (int i = 0; i < num; i++) {
		int row = -1;
		int col = -1;
